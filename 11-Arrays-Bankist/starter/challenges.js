...
const checkDogs = function (dogsJulia, dogsKate) {
  const juliaShallowCopy = [...dogsJulia].splice(1, 2);
  const kateShallowCopy = [...dogsKate];

  juliaShallowCopy.forEach(function (dog, index) {
    console.log(
      `${
        dog >= 3
  ...
  console.log(',,,,,');
  kateShallowCopy.forEach(function (dog, index) {
    console.log(
      `${
        dog >= 3
          ? `Dog number ${index + 1} is an adult, and is ${dog} years old`
...
const calcAverageHumanAge = function (ages) {
  ...
  const average = filter.reduce((acc, age) => acc + age) / filter.length;
...
"Active" means that the dog has 3 or more activities. Log to the console 
...
const swimmingAdjacentNumber = breeds.filter(breed =>
  breed.activities.includes('swimming')
).length;

console.log(swimmingAdjacentNumber);

const averageEveryDog = breeds.every(breed => breed.averageWeight >= 10);
console.log(averageEveryDog);

const activeDogs = breeds.some(breed => breed.activities.length >= 3);
console.log(activeDogs);

const averageWeightHeavyFetch = Math.max(
  ...breeds
    .filter(breed => breed.activities.includes('fetch'))
    .map(breed => breed.averageWeight)
);
console.log(averageWeightHeavyFetch);
...
Do NOT create a new array, simply loop over the array (We never did this before, 
...
if (SarahDog.curFood < SarahDog.recFood - 0.1 * SarahDog.recFood) {
...
const ownersTooMuch = dogs
  .filter(dog => dog.curFood > dog.recFood + 0.1 * dog.recFood)
  .flatMap(dog => dog.owners);
console.log('owners with too much food', ownersTooMuch);

const ownersTooLittle = dogs
  .filter(dog => dog.curFood < dog.recFood - 0.1 * dog.recFood)
  .flatMap(dog => dog.owners);

console.log('owners with too little food', ownersTooLittle);
...
const dogsGroup = Object.groupBy(
  dogs.filter(
    dog =>
      dog.curFood > dog.recFood + 0.1 * dog.recFood ||
      dog.curFood < dog.recFood - 0.1 * dog.recFood ||
  ...
  dog => {
    if (dog.curFood > dog.recFood + 0.1 * dog.recFood) {
      return 'too-much';
    }
    if (dog.curFood < dog.recFood - 0.1 * dog.recFood) {
      return 'too-little';
...
const dogsOwnersNumber = Object.groupBy(
  dogs,
  dog => `${dog.owners.length} owner${dog.owners.length !== 1 ? 's' : ''}`
);
console.log(dogsOwnersNumber);

const sortedArrayDogs = dogs.toSorted((a, b) => a.recFood - b.recFood);
console.log(sortedArrayDogs);
